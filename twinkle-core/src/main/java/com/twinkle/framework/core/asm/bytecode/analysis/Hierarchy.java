...
import com.twinkle.framework.core.asm.event.components.ClassHierarchyUpdateEvent;
import com.twinkle.framework.core.asm.graph.Graph;
...
/**
 * Graph model to represent the class inheritance of a loaded asmInput. <br>
...
public class Hierarchy implements Graph<ClassNode, ClassVertex> {
	...
	/**
	 * AsmInput to use for generating vertices from.
	 */
	private final AsmInput asmInput;

	public Hierarchy(AsmInput asmInput) {
	...
	public Set<ClassVertex> roots() {
		return getAsmInput().getClasses().values().stream()
	...
	/**
	 * @return AsmInput associated with the current hierarchy map.
	 */
	public AsmInput getAsmInput() {
		return asmInput;
	}
	...
	public ClassVertex getRoot(String name) {
		if (getAsmInput().classes.contains(name)) {
			ClassNode key = asmInput.getClass(name);
	...
	public boolean isLibrary(String owner, String name, String desc) {
		// Get classes that are considered "library" classes (not included in AsmInput)
		Stream<ClassVertex> hierarchy = getHierarchy(owner).stream();
		Stream<ClassVertex> libClasses = hierarchy.filter(vertex -> !asmInput.classes.contains(vertex.toString()));
	...
	private void setupChildLookup() {
		descendents.clear();
		for (ClassNode clazz : getAsmInput().getClasses().values()) {
