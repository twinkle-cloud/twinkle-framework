...
public class AutoComplete {
	 ...
	 * {@linkplain AsmInput#input the original reference} becomes unreachable.
	 */
	private static WeakReference<Optional<AsmInput>> lastInput = new WeakReference<>(null);

	/**
	 * A sorted list of class names for auto-completion,
	 * including {@linkplain Classpath#getSystemClassNames() the system's} and {@linkplain AsmInput#classes the current input's}.
	 */
	...
	private static Stream<String> classNames() {
		Optional<AsmInput> cachedInput = lastInput.get();
		Optional<AsmInput> currentInput = AsmInput.getOptional();
	...
	public static List<String> internalName(String part) {
		...
		return classNames()
				...
				// .sorted()    // AsmInput stream is already sorted
