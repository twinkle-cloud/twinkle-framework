package com.twinkle.framework.core.asm;

import com.twinkle.framework.core.asm.bytecode.analysis.Hierarchy;
import com.twinkle.framework.core.asm.bytecode.analysis.Verify;
...
public class AsmInput {
	...
	public AsmInput(Instrumentation instrumentation) throws IOException {
		this.input = null;
	...
	public AsmInput(File input) throws IOException {
		this.input = input;
	...
	public AsmInput(String _className, byte[] _content) throws IOException {
		this.input = null;
		this.instrumentation = null;
		current = Optional.of(this);
		InputBuilder builder = new InputBuilder(_className, _content);
		classes.addAll(builder.getClasses());
		resources.addAll(builder.getResources());
		classMap.putAllRaw(builder.getClassContent());
		resourceMap.putAll(builder.getResourceContent());
	...
	private void onClassRename(ClassRenameEvent event) {
		...
		if (node == null) {
			log.error("Fatal error occurred.", new RuntimeException("Failed to fetch updated ClassNode for remapped class: " + nameOriginal + " -> "
					+ nameRenamed));
		}
		...
		log.info("Rename " + nameOriginal + " -> " + nameRenamed);
	}
	...
	private void onFieldRename(FieldRenameEvent event) {
		...
		log.info("Rename " + fOwner + "." + fName + " -> " + fOwner + "." + fNameNew);
	}
			...
			for (ClassNode cn : classMap.values()) {
				...
				cn.accept(new ClassRemapper(updated, new Remapper() {
					...
					public String mapMethodName(final String owner, final String name, final String descriptor) {
						if (AsmConstant.LINKED_METHOD_REPLACE) {
					...
					private String rename(String owner, String name, String descriptor) {
						log.trace("Rename " + owner + "." + name + descriptor + " -> " + mNameNew);
						Bus.post(new ClassDirtyEvent(cn));
						classesWithUpdates.add(cn.name);
						return mNameNew;
					}
	...
	private void onSave(SaveStateEvent event) {
		for (String name : event.getClasses()) {
			...
			try {
				...
				log.info("Save state created for: " + name + " [" + classHistory.size() + " total states]");
			} catch (Exception e) {
				log.error("Encountered Exception", e);
			}
		}
	...
	private void onExportRequested(RequestExportEvent event) throws IOException {
		if (!Verify.isValid()) {
			...
			log.error("Denied export, please fix invalid bytecode");
			return;
		}
		...
		log.info("Exporting to " + event.getFile().getAbsolutePath());
		// write classes
		Set<String> modified = getModifiedClasses();
		log.info("Writing " + classes.size() + " classes...");
		log.info("\t" + modified.size() + " modified classes");
		for (String name : classes) {
			...
			if (modified.contains(name)) {
				try {
					...
					log.warn("Failed to export: '" + name + "' due to the following error: ");
					log.error("Encountered Exception", e);
				}
			}
		...
		log.info("Writing " + resources.size() + " resources...");
		...
		log.info("Exported to: " + event.getFile());
	...
	private void onAgentSave(RequestAgentSaveEvent event) {
		...
		try {
			...
			log.info("Redefinition complete.");
		} catch (Exception e) {
			log.error("Encountered Exception", e);
		}
		// clear dirty list
	 ...
	 * instance of "AsmInput" so that it can use it when fetching values posted by
		...
		instrumentation.addTransformer(
				(loader, className, classBeingRedefined, protectionDomain, classfileBuffer) -> {
					...
					try {
						...
						log.warn("Failed to load inst. class: " + className);
					}
					return classfileBuffer;
				}, true);
	}
	 ...
	 */
	private static Optional<AsmInput> current = Optional.empty();

	/**
	 * @return Current instance.
	 */
	public static AsmInput get() {
	 ...
	 */
	public static Optional<AsmInput> getOptional() {
		return current;
	}
