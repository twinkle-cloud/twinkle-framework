			...
			if (!checkBootstrapClass()) {
				...
				if (!Strings.isNullOrEmpty(System.getProperty("sun.boot.class.path"))) {
					try {
						...
						if (!checkBootstrapClass()) {
							log.warn("Bootstrap classes are (still) missing from the classpath scan!");
						}
					...
					log.warn("Recaf cannot acquire the classpath for bootstrap classes when running in Java 9 or above. " +
