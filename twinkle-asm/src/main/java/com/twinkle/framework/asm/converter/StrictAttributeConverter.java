...
public class StrictAttributeConverter implements AttributeConverter {
    ...
    public String normalize(String _attrName) {
        return ClassDesignerUtil.toBeanNormalForm(_attrName);
    }
    @Override
    ...
    protected void validateInternal(List<AttributeDef> _attrDefineList, boolean _validateNameFlag) {
        Map<Integer, AttributeDef> tempMap1 = new HashMap(_attrDefineList.size());
        Map<String, AttributeDef> tempMap2 = new HashMap(_attrDefineList.size());
        _attrDefineList.stream().parallel().forEach(item ->{
            ...
            AttributeDef tempAttrDef = tempMap1.put(tempAttrDefineName.hashCode(), item);
            if (tempAttrDef != null) {
